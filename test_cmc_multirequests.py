...
from settings import USE_MOCK, LOG_TO_DB, MAX_PERCENTILE, MAX_TIME_OF_RESPONSE, \
    MIN_RPS, MAX_SIZE_OF_RESPONSE, UTC_OFFSET
...
class TestCmcMultirequest(ut.TestCase):
    ...
    def test_stage1_time_of_response(self):
        ...
        if not self.test_table:  # Cheking if table is void
            raise KeyError("Invalid Api-key")  # Void table means broken API-Key
    ...
    def tearDownClass(cls) -> None:
        if LOG_TO_DB:
            ...
            db_req('log.db',
                   f"INSERT INTO log (data, time) values('{str(datetime.now() + timedelta(hours=UTC_OFFSET))}',"
                   f" '{str(cls.test_table)}');")
